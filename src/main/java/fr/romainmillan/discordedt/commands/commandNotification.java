        ...
        if(e.getName().equals("notification")){
            ...
            if(e.getMember().getRoles().contains(sudo)){
                ...
                if(action != null){
                    ...
                    Sentry.getInstance().toLog(PluginName.EDT.getMessage(), "Notifications supprimée", LogState.SUCCESSFUL, e.getMember(), e.getGuild());
                }else {
                    ...
                    Sentry.getInstance().toLog(PluginName.EDT.getMessage(), "Notification de cour pour le groupe `"+groupe+"`", LogState.SUCCESSFUL, e.getMember(), e.getGuild());
                }
            }else {
                ...
                Sentry.getInstance().toLog(PluginName.EDT.getMessage(), LoggerMessages.USER_NO_PERMISSION.getMessage(), LogState.ERROR, e.getMember(), e.getGuild());
            }
        }
    }
}
